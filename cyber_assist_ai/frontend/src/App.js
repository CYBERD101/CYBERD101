...
const MouseFollower = React.memo(({ color }) => {
  ...
  useEffect(() => {
    const handleMouseMove = (e) => {
      if (followerRef.current) {
        // Use translate3d for hardware acceleration and direct DOM manipulation
        // to bypass React re-renders on every mouse move.
  ...
  return (
    <div
      ref={followerRef}
      className="fixed pointer-events-none z-50 w-8 h-8 rounded-full border opacity-50 will-change-transform transition-transform duration-75"
      style={{
        borderColor: color,
