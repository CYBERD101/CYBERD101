 ...
 * Refactored to use direct DOM manipulation for mouse tracking.
 * This avoids high-frequency React re-renders (60+ times per second)
 * during mouse movement, ensuring smooth performance even during
 * heavy AI response streaming.
 */
  ...
  useEffect(() => {
    const handleMouseMove = (e) => {
      ...
      if (followerRef.current) {
        // Using translate3d for hardware-accelerated movement
...
function App() {
  // Optimization: themeColor is memoized to prevent unstable primitive props
  // from triggering unnecessary re-renders of memoized child components
  ...
  const themeColor = useMemo(() => status === 'processing' ? '#f59e0b' : status === 'alert' ? '#ef4444' : '#00FF41', [status]);
